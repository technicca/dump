...
options=("CentOS / RHEL Based" "Debian / Ubuntu-based" "Fedora" "Arch Linux" "Another")
singleselectn "${options[@]}"
selected_os="${options[$?]}"
echo "Selected OS: $selected_os"

my_options=(   "Disable password authentication"  "Disable root account remote login"  "Limit maximum number of SSH authentication attempts"  "Change the SSH port"  "Install Fail2Ban"  "IPtables ssh security"  "Implement two-factor authentication")
preselection=( "true"      "true"      "false"     "false"     "false"    "false"    "false")
...
for option in "${my_options[@]}"; do
    if [[ ${result[idx]} == "true" ]] && [[ $option =~ "authentication"|"login"|"SSH"|"port"|"Implement two-factor" ]]; then
        ssh_selected=true
    fi
    ((idx++))
done

echo -e "\nScript Execution : $(date)", "Selected OS: $selected_os" | tee -a log.txt
